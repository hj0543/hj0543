# ☁️ Code Haneul

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=venom&color=auto&height=200&section=header&text=Code%20Haneul&fontSize=70" />
</div>

## 🎓 Introduction
> **""**

저는 화력운용장교로서의 **책임감**과 학원 팀장으로서의 **시스템 운영 경험**을 갖추고 있습니다. 
이제 SSAFY에서 더 견고하고 효율적인 코드를 설계하는 개발자로 성장하고 있습니다.

* **Career**
  * 🎖️ **ROK Army (2019.03 - 2021.06)**: 육군 중위 전역 (화력운용장교) - 위기 관리 및 체계적 프로세스 운용
  * 🏫 **Ildeung Math Academy (2023.10 - 2025.12)**: 팀장 및 시스템 개발 - 교육 현장의 불편함을 IT 기술로 해결 (학원 관리 시스템 기획 및 운영)
* **Education**: SSAFY 15th (Gumi Campus)

---

## 🛠 Tech Stack
<img src="https://img.shields.io/badge/java-007396?style=for-the-badge&logo=java&logoColor=white"> <img src="https://img.shields.io/badge/python-3776AB?style=for-the-badge&logo=python&logoColor=white">
<img src="https://img.shields.io/badge/springboot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"> <img src="https://img.shields.io/badge/mysql-4479A1?style=for-the-badge&logo=mysql&logoColor=white">

---

## 📑 Development Philosophy
멘토링을 통해 정립한 저의 개발 원칙입니다. 모든 프로젝트에 아래 과정을 준수합니다.

1. **Issue-Driven Development**: 모든 기능 구현과 버그 수정은 Issue 발행부터 시작합니다. (WBS 기반 관리)
2. **Commit with 'Why'**: 무엇을 했는지가 아닌, '왜' 이 코드가 필요한지를 커밋 메시지에 담습니다.
3. **Strict Workflow**: Git-Flow 전략을 지향하며, PR을 통한 코드 리뷰와 개선 과정을 기록합니다.
4. **User-Centric Solution**: 기술적 도전이 실제 사용자에게 어떤 가치를 주는지 고민합니다.

---

### 🌌 My Grass Garden
![3D Grass Wave](./profile-3d-contrib/profile-night-rainbow.svg)

### 🏆 Solved.ac Profile
<img src="http://mazandi.herokuapp.com/api?handle=hj0543&theme=dark" />

### 📫 Contact Me
<a href="mailto:hj0543@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white">
</a>
